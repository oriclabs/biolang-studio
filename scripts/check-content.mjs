...
const failures = [];
if (readFileSync(path.join(root, "public", "CNAME"), "utf8").trim() !== "studio.lang.bio") failures.push("public CNAME must publish Studio at studio.lang.bio");
...
const lessonDataSchema = JSON.parse(readFileSync(path.join(root, "public", "schemas", "lesson-data-lock-v1.schema.json"), "utf8"));
if (lessonDataSchema.$id !== "https://studio.lang.bio/schemas/lesson-data-lock-v1.schema.json" || lessonDataSchema.properties?.schema?.const !== 1 || lessonDataSchema.properties?.kind?.const !== "biolang-lesson-data-lock" || !lessonDataSchema.required?.includes("project") || !lessonDataSchema.required?.includes("files")) failures.push("lesson data-lock schema identity or required fields differ from runtime v1");
...
console.log(`${catalog.length} lesson manifest, all workspace schemas, the run-record schema, and the lesson data-lock schema checked; remote datasets are references only.`);
