...
When writing an explanation cell, only explicit fenced `biolang` or `bl` blocks become executable; inline backticks remain prose. Every code cell has **BioLang** and **JavaScript** tabs, synchronized with the notebook-level **Code** selector. Direct translations use ordinary JavaScript variables, field access, expressions, and calls such as `await bl.mean(values)`; the constrained editor rejects browser globals, dynamic code, mutation, and unsupported JavaScript before execution. Translations that need the structural frontend remain read-only. CodeMirror supplies line numbers, parser diagnostics, completion for built-ins and notebook variables, and run shortcuts. Edited direct JavaScript is stored readably in a `javascript+biolang` companion fence while the BioLang fence remains the portable canonical program understood by the CLI. Direct JavaScript editing and execution use the Browser kernel; Desktop and SOMER run the canonical BioLang tab, while the standalone JavaScript SDK can submit generated BioLang programs to SOMER. The selected language is remembered, and `?lang=js` or `?lang=bl` can choose the initial view in a shared link. **Copy** copies the displayed language without running it. Run records disclose the selected frontend and hash both the frontend and the BioLang actually executed. The Variables inspector is collapsed by default and shows its current item count, leaving the sidebar focused on lessons and data.
