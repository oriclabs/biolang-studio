...
When writing an explanation cell, only explicit fenced `biolang` or `bl` blocks become executable; inline backticks remain prose. Every existing code cell has **BioLang** and **JavaScript** tabs, synchronized with the notebook-level **Code** selector. BioLang remains the canonical editable lesson source; the JavaScript tab generates complete `await bl.run(...)` session code and makes explicit that the same BioLang kernel performs the scientific work. The selected language is remembered, and `?lang=js` or `?lang=bl` can choose the initial view in a shared link. **Copy** copies the currently displayed language without running it. Run, Run step, and Run all use the selected frontend; JavaScript is compiled to the exact canonical BioLang payload before WASM, Desktop, or SOMER executes it. Run records disclose the selected frontend and hash both the frontend and executed BioLang source. This preserves lesson results and verified manifest checksums while making the execution path auditable. The Variables inspector is collapsed by default and shows its current item count, leaving the sidebar focused on lessons and data.
